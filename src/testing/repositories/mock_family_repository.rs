...
impl FamilyRepository for MockFamilyRepository {
    async fn get_families_for(&self, _conn: &mut dyn AsPgConn, _username: &str) -> Result<Vec<Family>, RepositoryError> {
        if self.should_error { return Err(RepositoryError::NotFound); }
        Ok(self.families.clone())
    }

    async fn get_family_by_name(&self, _conn: &mut dyn AsPgConn, username: &str, name: &str) -> Result<Family, RepositoryError> {
        if self.should_error { return Err(RepositoryError::NotFound); }
        self.families
            .iter()
    ...
    async fn is_family_member(&self, _username: &str, _family_id: i32) -> Result<bool, RepositoryError> {
        if self.should_error { return Err(RepositoryError::NotFound); }
        Ok(self.is_family_member)
    }

    async fn create_family(&self, _conn: &mut dyn AsPgConn, username: &str, command: &CreateFamilyCommand) -> Result<Family, RepositoryError> {
        if self.should_error { return Err(RepositoryError::NotFound); }
        Ok(Family {
