...
pub async fn check_magic_list_access<DB: DbConnection>(
    ...
    let magic_list = state
        ...
        .await
        .map_err(|e| map_find_error(magic_list_id, e))?;

    let authorized = magic_list.owner_username == username
        || is_shared_family_member(state, username, &magic_list)
            .await
            .map_err(|source| CheckMagicListAccessError::Repository { magic_list_id, source })?;

    authorized
        .then_some(())
        .ok_or(CheckMagicListAccessError::AccessDenied { magic_list_id })
}

fn map_find_error(magic_list_id: i32, error: RepositoryError) -> CheckMagicListAccessError {
    match error {
...
async fn is_shared_family_member<DB: DbConnection>(
    state: &web::Data<ActixState<DB>>,
    username: &str,
    magic_list: &MagicList,
) -> Result<bool, RepositoryError> {
    let Some(family_id) = magic_list
        .family_id
        .filter(|_| magic_list.visibility == Visibility::Shared)
    else {
        return Ok(false);
    };
